...
# logistic_grad: Gradient of the logistic function above. Crucial for the optimization implemented in the stochastic gradient algorithm.
...
# Below is an implementation of stochastic gradient descent--a common approach to optimizing vector functions 
...
# to learn. Specifically, we want the optimal parameter which minimizes the cross entropy between our logistic model and the result of the game. 

def stochastic_grad(inputs, outputs, parameter, step_size, cut_off, train_size, convergence):
	start = time.time()
	# A list of the integers from 1 to 'training_size' (number of training samples) is created, so that on each iteration, we randomly shuffle these indices and
	# and update the parameter according to each random game sample at each iteration. k is the iteration of stochastic gradient descent, 
	# while i is the ith random sample chosen from the kth randomly shuffled training data. 
	indices = list(range(train_size))
	grad_i = np.zeros(feature_size)
	param_i = parameter
	k = 0
	grad_norm = 0.0
	# Termination occurs when one of the following holds: 1) The parameter has converged sufficiently close to the minimum parameter, as measured by 
	# the norm of the gradient 'grad_norm' being < convergence. 2) Maximum iteration 'cut_off' has reached. This has to be included because convergence is not always guaranteed. 
	while (((grad_norm > convergence) or k == 0) and (k < cut_off)):
		# Indices randomly shuffled. 
		random.shuffle(indices)
		# Each ith raining sample from the randomly sorted training data is gradient-analyzed, and the parameter is updated accordingly.
		for i in range(train_size):
			rand_index = indices[i]
			rand_input = inputs[rand_index]
			rand_output = outputs[rand_index]
			grad_old = grad_i
			grad_i = rand_input * (logistic(rand_input,param_i) - rand_output)
			param_i = param_i - (step_size * grad_i)
...
# a stochastic gradient algorithm is used. For stochastic gradient descent, step size is typically somewhat of a shot in the dark.
# The provided value seems to work decently well.
# Convergence is somewhat arbitrary, but dictates when the stochastic gradient will terminate. More specifically, once the gradient norm is less than the convergence threshold 'convergence', 
...
# 'step_size': Scalar factor which scales how strongly the parameter changes in response to the stochastic gradient at each iteration.
# 'cut_off': Maximum iteration, at which time the stochastic gradient descent terminates and returns its current estimate of the optimal (minimizing) parameter 'param_op'. 
step_size = 1.0e-2
cut_off = 10000
convergence = 1.0e-4
param_opt = stochastic_grad(inputs, results, param_init, step_size, cut_off, train_size, convergence)
